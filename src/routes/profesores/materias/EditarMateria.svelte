<script lang="ts">
    ...
    let formData: MateriaUpdate = {
        nombre_materia: materia.nombre_materia,
        nivel: (materia.nivel || "secundaria").toLowerCase(),
    };
    ...
    async function handleSubmit() {
        ...
        try {
            loading = true;
            error = "";
            await materiasService.updateMateria(materia.id_materia, formData);
            dispatch("save");
        } catch (err: any) {
            error =
                err.response?.data?.detail || "Error al actualizar la materia";
        } finally {
            loading = false;
        }
    }
...
<form on:submit|preventDefault={handleSubmit} class="form-content">
    ...
    <div class="form-group">
        ...
        <input
            type="text"
            id="nombre"
            bind:value={formData.nombre_materia}
